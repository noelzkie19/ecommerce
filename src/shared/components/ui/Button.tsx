...
export function Button({ variant = 'primary', size = 'md', loading, className, children, disabled, ...props }: Props) {
  return (
    <button
      ...
      className={cn('w-full rounded-xl font-medium transition disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2', variants[variant], sizes[size], className)}
      {...props}
    >
