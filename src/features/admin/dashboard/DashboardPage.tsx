            ...
            {recentOrders.length > 0 ? (
              recentOrders.map((order: RecentOrder) => (
                <tr key={order.id} className="text-sm hover:bg-gray-50 transition">
                  ...
                  <td className="p-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${STATUS_STYLES[order.status.toLowerCase()] ?? 'bg-gray-100 text-gray-700'}`}>
                      {order.status}
                    </span>
                  </td>
                  <td className="p-4 text-gray-700 font-medium">₱{order.total.toLocaleString()}</td>
                </tr>
              ))
            ) : (
              <tr>
