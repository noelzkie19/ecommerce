"use client";

import { useState } from "react";
import { Plus, Search, Loader2 } from "lucide-react";
import { useCommunityLinks } from "./hooks/useCommunityLinks";
import { CommunityTable } from "./components/CommunityTable";
import CommunityFormModal from "./components/CommunityFormModal";
import { DeleteConfirmModal } from "./components/DeleteConfirmModal";
import { CommunityPagination } from "./components/CommunityPagination";
import {
  CommunityLink,
  CommunityLinkCategory,
  COMMUNITY_LINK_CATEGORIES,
} from "@/types/community.types";

export default function CommunityPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const [category, setCategory] = useState<CommunityLinkCategory | "">("");
  const [isActive, setIsActive] = useState<boolean | "">("");

  const [showForm, setShowForm] = useState(false);
  const [editingLink, setEditingLink] = useState<CommunityLink | null>(null);
  const [deletingLink, setDeletingLink] = useState<CommunityLink | null>(null);

  const { links, meta, isLoading, error, refetch } = useCommunityLinks({
    page,
    limit: 10,
    search: search || undefined,
    category: category || undefined,
    isActive: isActive === "" ? undefined : isActive,
  });

  const handleEdit = (link: CommunityLink) => {
    setEditingLink(link);
    setShowForm(true);
  };

  const handleDelete = (link: CommunityLink) => {
    setDeletingLink(link);
  };

  const handleFormClose = () => {
    setShowForm(false);
    setEditingLink(null);
  };

  const handleFormSuccess = () => {
    refetch();
  };

  const handleDeleteSuccess = () => {
    refetch();
  };

  return (
    <div className="p-8 space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">
            Community Links
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Manage community links displayed on the affiliate dashboard
          </p>
        </div>
        <button
          onClick={() => setShowForm(true)}
          className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors font-medium text-sm"
        >
          <Plus size={18} />
          Add New
        </button>
      </div>

      {/* Filters */}
      <div className="flex flex-wrap items-center gap-4 p-4 bg-white rounded-xl border border-gray-200">
        <div className="flex-1 min-w-[200px]">
          <div className="relative">
            <Search
              size={18}
              className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"
            />
            <input
              type="text"
              placeholder="Search links..."
              value={search}
              onChange={(e) => {
                setSearch(e.target.value);
                setPage(1);
              }}
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
            />
          </div>
        </div>

        <select
          value={category}
          onChange={(e) => {
            setCategory(e.target.value as CommunityLinkCategory | "");
            setPage(1);
          }}
          className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white min-w-[150px]"
        >
          <option value="">All Categories</option>
          {COMMUNITY_LINK_CATEGORIES.map((cat) => (
            <option key={cat} value={cat}>
              {cat.charAt(0).toUpperCase() + cat.slice(1)}
            </option>
          ))}
        </select>

        <select
          value={isActive === "" ? "" : String(isActive)}
          onChange={(e) => {
            setIsActive(e.target.value === "" ? "" : e.target.value === "true");
            setPage(1);
          }}
          className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white min-w-[120px]"
        >
          <option value="">All Status</option>
          <option value="true">Active</option>
          <option value="false">Inactive</option>
        </select>
      </div>

      {/* Error State */}
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl px-5 py-4">
          {error}
        </div>
      )}

      {/* Loading State */}
      {isLoading && (
        <div className="flex items-center justify-center py-12">
          <Loader2 size={32} className="animate-spin text-emerald-600" />
        </div>
      )}

      {/* Table */}
      {!isLoading && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <CommunityTable
            links={links}
            onEdit={handleEdit}
            onDelete={handleDelete}
          />
          {meta && <CommunityPagination meta={meta} onPageChange={setPage} />}
        </div>
      )}

      {/* Form Modal */}
      {showForm && (
        <CommunityFormModal
          link={editingLink}
          onClose={handleFormClose}
          onSuccess={handleFormSuccess}
        />
      )}

      {/* Delete Modal */}
      {deletingLink && (
        <DeleteConfirmModal
          link={deletingLink}
          onClose={() => setDeletingLink(null)}
          onSuccess={handleDeleteSuccess}
        />
      )}
    </div>
  );
}
